...
import { redigir, urlHttpValida } from '../src/seguranca.ts'
...
const wf = await resposta.json() as { name: string; active?: boolean; settings?: unknown; nodes: { name: string; type: string; typeVersion?: number; disabled?: boolean; parameters?: unknown }[]; connections: Record<string, { main?: { node: string }[][] }> }
